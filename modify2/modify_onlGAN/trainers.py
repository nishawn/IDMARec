...
class Trainer:
    ...
    def sample_from_generator(self, seq_out_A, seq_out_B, pos_ids,  generator_B):
        ...
        return sampled_neg_ids_B,  pos_idx_B, neg_idx_B, mask_idx, istarget_B
...
class ICLRecTrainer(Trainer):
    ...
    def _instance_cl_one_pair_contrastive_learning(self, inputs, intent_ids=None):
        ...
        if self.args.de_noise:
            ...
            cl_loss_B = self.cf_criterion(cl_output_slice_B[0], cl_output_slice_B[1], intent_ids=intent_ids)
            ...
            cl_loss_B = self.cf_criterion(cl_output_slice_B[0], cl_output_slice_B[1], intent_ids=None)
    ...
    def _pcl_one_pair_contrastive_learning(self, inputs, intents, intent_ids):
        ...
        if self.args.de_noise:
            ...
            cl_loss_B = self.pcl_criterion(cl_output_slice_B[0], cl_output_slice_B[1], intents=intents,
    ...
    def iteration(self, epoch, dataloader, cluster_dataloader=None, full_sort=True, train=True):
        ...
        if train:
            ...
            for i, (rec_batch, seq_class_label_batches) in rec_cf_data_iter:
                """
                ...
                sampled_neg_ids_B, pos_idx_B, neg_idx_B,\
                mask_idx, istarget_B = self.sample_from_generator( sequence_output_B,
                                                                              target_pos,
                                                                              self.generator_B)


                gen_loss = self.generator_cross_entropy(sequence_output_B, target_pos, target_neg, self.generator_B) \
                ...
                dis_loss = self.discriminator_cross_entropy(disc_logits_B, pos_idx_B, neg_idx_B, mask_idx, istarget_B,
                ...
                if self.args.contrast_type == "InstanceCL":
                        cl_loss = self._instance_cl_one_pair_contrastive_learning(
                            input_ids, intent_ids=seq_class_label_batches
                        )
                        ...
                        if epoch >= self.args.warm_up_epoches:
                            ...
                            cl_loss = self._pcl_one_pair_contrastive_learning(
                                input_ids, intents=seq2intents, intent_ids=intent_ids
                            )
                        ...
                        if epoch < self.args.warm_up_epoches:
                            ...
                            cl_losses.append(self.args.cf_weight * cl_loss1)
                            if self.args.seq_representation_type == "mean":
                                sequence_output = torch.mean(sequence_output_B, dim=1, keepdim=False)
                            sequence_output = sequence_output.view(sequence_output.shape[0], -1)
                            sequence_output = sequence_output.detach().cpu().numpy()
                            # query on multiple clusters
                            for cluster in self.clusters:
            ...
            if full_sort:
                ...
                for i, batch in rec_data_iter:
                    ...
                    recommend_output = self.discriminator_B(input_ids)
