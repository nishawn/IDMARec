...
from models import SASRecModel, SASRecAttributeModel, SASRecUserItemPromptModel, SASRecUserPropmtModel
from utils import EarlyStopping, get_user_seqs, get_item2attribute_json, check_path, set_seed

#尝试删除原有的数据增强，只保留两个GAN进行多个对比
#尝试将属性和item这里重新写，属性相似的item构建一个图？
#构建user-item交互图
...
# update_to_module_parameters with gen or discriminator
...
def main():
    ...
    args.log_file = os.path.join(args.output_dir, args_str + "testMultiGAN_embedonly.txt")
    ...
    # generator_A = SASRecUserPropmtModel(args=args)
    ...
    # trainer = ICLRecTrainer(generator_A, generator_B, discriminator_A, discriminator_B,
    #                     train_dataloader, cluster_dataloader, eval_dataloader, test_dataloader, args)

    trainer = ICLRecTrainer(generator_B, discriminator_B,
                            train_dataloader, cluster_dataloader, eval_dataloader, test_dataloader, args)
