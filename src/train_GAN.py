...
class Trainer:
    ...
    # def sample_from_generator(self, seq_out_A, seq_out_B, pos_ids, generator_A, ):
    ...
    #     istarget_B = (pos_ids > 0).view(pos_ids.size(0) * .args.max_seq_length).float()  # [batch*seq_len]
    #
    #     K = int(self.args.item_size * self.args.item_sample_ratio) - 1
    #     neg_ids = random.sample([i for i in range(1, self.args.item_size)], K)
    #     neg_ids = torch.tensor(neg_ids, dtype=torch.long).to(self.device)
    #     neg_emb_A = generator_A.item_embeddings(neg_ids)
    #     neg_emb_B = .item_embeddings(neg_ids)
    ...
    #     pos_idx_B = (pos_ids == sampled_neg_ids_B).view(pos_ids.size(0) * .args.max_seq_length).float()
    #     neg_idx_B = (pos_ids != sampled_neg_ids_B).view(pos_ids.size(0) * .args.max_seq_length).float()
    #
    ...
    # def CL_generator_cross_entropy(self, seq_out_A, seq_out_B, pos_ids, multi_neg_ids, generator_A, ):
    #     # [batch seq_len hidden_size]
    #     pos_emb_A = generator_A.item_embeddings(pos_ids)
    #     pos_emb_B = .item_embeddings(pos_ids)
    #     # [batch*seq_len hidden_size]
    ...
    #         test_item_emb_B = .item_embeddings.weight
    ...
    def predict_sample(self, seq_out, test_neg_sample):
        ...
        test_item_emb = self.discriminator_A.item_embeddings(test_neg_sample)
        # [batch hidden_size]
    ...
    def predict_full(self, seq_out):
        ...
        test_item_emb = self.discriminator_A.item_embeddings.weight
        # [batch hidden_size ]
...
class ELECITY(Trainer):
    ...
    def _pcl_one_pair_contrastive_learning(self, inputs, intents, intent_ids):
        ...
        cl_batch = cl_batch.to(self.device)
        cl_sequence_output = self.generator_A(cl_batch)
        # cl_sequence_output_B = self.(cl_batch)
        if self.args.seq_representation_type == "mean":
            cl_sequence_output = torch.mean(cl_sequence_output, dim=1, keepdim=False)
        ...
        if train:
            ...
            if self.args.contrast_type in ["IntentCL", "Hybrid"] and epoch >= self.args.warm_up_epoches:
                ...
                for i, (rec_batch, _, _) in rec_cf_data_iter:
                    ...
                    # sequence_output_B = self.(input_ids)
                    # average sum
                    if self.args.seq_representation_type == "mean":
            ...
            for i, (rec_batch, cl_batches, seq_class_label_batches) in rec_cf_data_iter:
                ...
                sampled_neg_ids_A, pos_idx_A,  neg_idx_A, mask_idx, istarget_A,= self.sample_from_generator(sequence_output_A,
                                                                              target_pos,
                                                                              self.generator_A,
                                                                              )
